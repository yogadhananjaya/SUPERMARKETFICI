...
void menuLaporan() {
    int selected = 0;
    const char *menuItems[] = {"Laporan Penjualan", "Laporan Pembelian", "Kembali"};

    drawBaseLayout("PUSAT LAPORAN"); drawHomeLogo();
            ...
            if(selected == 0) { // PENJUALAN
                ...
                for(int i=0; i<totalPenjualan; i++) {
                     ...
                     if(m==bln && y==thn) {
                         ...
                         // Get Product Name
                         strcpy(pn, "Item Hapus");
                         for(int p=0; p<totalProduk; p++) if(dbProduk[p].id == dbPenjualan[i].idProduk) strcpy(pn, dbProduk[p].nama);

                         gotoxy(tx, startY+3+row);
                         printf(" %-12s %-15s %-30s Rp %12s ", dbPenjualan[i].tanggal, dbPenjualan[i].noFaktur, pn, rp);
                         grandTotal += dbPenjualan[i].totalHarga;
                         row++; if(row >= 10) break;
                     }
                }
            } else { // PEMBELIAN
                ...
                for(int i=0; i<totalPembelian; i++) {
                     ...
                     if(m==bln && y==thn) {
                         ...
                         strcpy(pn, "Unknown");
                         for(int p=0; p<totalProduk; p++) if(dbProduk[p].id == dbPembelian[i].idProduk) strcpy(pn, dbProduk[p].nama);

                         gotoxy(tx, startY+3+row);
                         printf(" %-12s %-30s %-10d Rp %12s ", dbPembelian[i].tanggal, pn, dbPembelian[i].jumlah, rp);
