...
void getValidatedString(char *buffer, int maxLen, int x, int y) {
    while(1) {
        ...
        if(strlen(buffer) > 0) {
            ...
            showErrorAndWait(x, y, "Wajib diisi!");
        }
    }
}
...
long getValidatedNumber(int x, int y) {
    ...
    while(1) {
        ...
        if (strlen(buffer) == 0) {
            showErrorAndWait(x, y, "Tidak boleh kosong!");
        }
        else if (!isNumeric(buffer)) {
            showErrorAndWait(x, y, "Harus angka!");
        }
        else {
            result = atol(buffer);
            if(result < 0) {
                showErrorAndWait(x, y, "Tidak boleh negatif!");
            } else {
                gotoxy(x, y+1); printf("                                         ");
                break;
            }
    ...
    while(1) {
        ...
        if (strlen(buffer) == 0) {
            showErrorAndWait(x, y, "Tidak boleh kosong!");
             ...
             showErrorAndWait(x, y, "Kontak harus angka!");
        }
        else {
            gotoxy(x, y+1); printf("                                         ");
            break;
    ...
    *loggedIndex = -1;
    ...
    while(1) {
        ...
        for(int y=loginY+1; y<loginY+boxH; y++) {
            gotoxy(loginX, y); printf("%c", 186); gotoxy(loginX+boxW, y); printf("%c", 186);
        }
        ...
        drawNavigationLegend("[TAB] Lihat Password | [ENTER] Submit | [ESC] Keluar Program");
        ...
        if (strcmp(username, "admin") == 0 && strcmp(password, "admin") == 0) {
            gotoxy(loginX + 5, loginY + 8); printf("Login Admin Berhasil!");
            Sleep(800); setCursorVisible(FALSE);
            return 1;
        }

        int found = 0;
        for(int i=0; i<totalKaryawan; i++) {
            if(strcmp(username, dbKaryawan[i].username) == 0 &&
               strcmp(password, dbKaryawan[i].password) == 0) {
                *loggedIndex = i;
                found = 1;
                break;
            }
        ...
        if(found) {
            gotoxy(loginX + 5, loginY + 8); printf("Login Karyawan Berhasil!");
            Sleep(800); setCursorVisible(FALSE);
            return 2;
        } else {
            gotoxy(loginX + 5, loginY + 8);
            printf("Gagal! Cek Username/Password.");
            Beep(500, 200);
            while(1) {
                int key = getch();
